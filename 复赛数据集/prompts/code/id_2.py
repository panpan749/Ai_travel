...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    model.hotel_type = pyo.Constraint(
        rule=lambda m: sum(
            ...
            if ('三星级' in m.hotel_data[h]['type'] or '四星级' in m.hotel_data[h]['type'] or '五星级' in m.hotel_data[h]['type']) and '早餐' in m.hotel_data[h]['feature']
        ) == 1
    )
...
def generate_poi(model, intra_city_trans):
    ...
    return {
        "query_id": "2",
        "query": "2025年7月1日上午，我将从武汉大学地铁站启程前往厦门，开启为期3天的旅行，并于7月3日晚搭乘高铁返程。出发交通指定搭乘D3286次高铁，本次旅行预算控制在20000元以内，期望能深入体验厦门的本地文化与自然风景。请优先安排评分高的景点、餐厅与住宿，入住三星级及以上宾馆，要求含早餐，且至少吃一次茶艺馆，并确保每日动线紧凑、交通方便，提升整体出行舒适度。",
