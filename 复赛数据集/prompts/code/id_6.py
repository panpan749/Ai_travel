...
budget = 34300
start_date = "2025年04月04日"
end_date = "2025年04月07日"
travel_days = 4
peoples = 2

def fetch_data():
    url = "http://127.0.0.1:12457"
    cross_city_train_departure = requests.get(url + f"/cross-city-transport?origin_city={origin_city}&destination_city={destination_city}").json()
    cross_city_train_back = requests.get(url + f"/cross-city-transport?origin_city={destination_city}&destination_city={origin_city}").json()
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    def obj_rule(model):
        ...
        return (peoples + 1) //2 * hotel_cost + transport_cost + attraction_cost + restaurant_cost + train_departure_cost + train_back_cost
    ...
    model.rest_food = pyo.Constraint(
        rule=lambda m: sum(
            ...
            for r in model.restaurants
            for d in days
            if '宫保鸡丁' in model.rest_data[r]['recommended_food']
        ) >= 1
    )
    ...
    model.attr_name = pyo.Constraint(
        rule=lambda m: sum(
            ...
            for a in model.attractions
            for d in days
            if '龙潭公园' in model.attr_data[a]['name']
        ) >= 1
    )
...
def generate_poi(model, intra_city_trans):
    ...
    return {
        "query_id": "6",
        "query": "我计划于2025年04月04日至2025年04月07日从洛阳市前往北京市开展为期4天的3人旅行,有1名孩童,总预算为34300元,需满足以下需求:行程中须包含龙潭公园等景点,并体验宫保鸡丁。2025年04月04日早上出发,2025年04月07日晚返回。尽量减少不必要的开销。",
