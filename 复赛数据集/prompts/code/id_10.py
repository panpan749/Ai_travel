...
budget = 33100
start_date = "2025年07月22日"
end_date = "2025年07月24日"
travel_days = 3
peoples = 4
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    train_departure_dict = {t['train_number']: t for t in cross_city_train_departure if t['train_number'] == 'G3078'}
    ...
    def obj_rule(model):
        ...
        hotel_cost = (peoples + 1)//2 * sum(model.select_hotel[h] * model.hotel_data[h]['cost'] * (travel_days - 1)
                         for h in model.accommodations)
        ...
        return rating - (peoples + 1) //2 * hotel_cost - transport_cost - peoples * (
    ...
    model.rest_name1 = pyo.Constraint(
        rule=lambda m: sum(
            ...
            for r in model.restaurants
            for d in days
            if '猫咖啡' in model.rest_data[r]['recommended_food']
        ) >= 1
    )
    ...
    model.rest_name1 = pyo.Constraint(
        rule=lambda m: sum(
            ...
            for r in model.restaurants
            for d in days
            if '素斋' in model.rest_data[r]['recommended_food']
        ) >= 1
    )
...
def get_cost(model, selected_attr, selected_rest, departure_trains, back_trains, selected_hotel, day, intra_city_trans):
    ...
    if day != travel_days:
        daily_cost += (peoples + 1) // 2 * selected_hotel['cost']
    if day == 1:
...
def generate_poi(model, intra_city_trans):
    ...
    return {
        "query_id": "10",
        "query": "我计划于2025年07月22日至2025年07月24日从广州市前往苏州市，开启为期3天的高性价比4人休闲之旅，总预算控制在33100元以内。旅行要求包括：全程入住三星级及以上标准酒店，安排一次猫咖啡与素斋的本地餐饮体验；2025年07月22日早上出发，2025年07月24日晚返回，去程高铁指定为G3078次列车。",
