...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    model.feature_rest2 = pyo.Constraint(
        rule=lambda m: sum(
            model.select_rest[d, r]
            for r in model.restaurants
            for d in days
            if '火锅' in model.rest_data[r]['type'] or '火锅' in model.rest_data[r]['recommended_food']
        ) >= 1
    )
...
def generate_poi(model, intra_city_trans):
    ...
    return {
        "query_id": "4",
        "query": "2025年06月22日至2025年06月24日，我计划从广州市前往成都市旅游，希望品尝当地的特色火锅，用餐时间与排队时间之和不能超过120分钟，不限制预算但是尽量减少开销。",
