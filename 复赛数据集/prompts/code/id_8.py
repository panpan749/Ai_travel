...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    def obj_rule(model):
        ...
        train_departure_cost = sum(model.select_train_departure[t] * model.train_departure_data[t]['duration']
                for t in model.train_departure)
        train_back_cost = sum(model.select_train_back[t] * model.train_back_data[t]['duration']
                for t in model.train_back)
        return transport_cost + train_departure_cost + train_back_cost
    ...
    model.hotel_type = pyo.Constraint(
        rule = lambda m: sum(
            ...
            if '五星级' in m.hotel_data[h]['type'] or '四星级' in m.hotel_data[h]['type']
        ) == 0
    )
