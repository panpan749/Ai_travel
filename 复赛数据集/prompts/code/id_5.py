...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    model.rest_name1 = pyo.Constraint(
        rule=lambda m: sum(
            ...
            for r in model.restaurants
            for d in days
            if '黄焖鸡米饭' in model.rest_data[r]['recommended_food']
        ) >= 1
    )
    ...
    model.rest_name1 = pyo.Constraint(
        rule=lambda m: sum(
            ...
            for r in model.restaurants
            for d in days
            if '茶叶蛋' in model.rest_data[r]['recommended_food']
        ) >= 1
    )
...
def generate_poi(model, intra_city_trans):
    ...
    return {
        "query_id": "5",
        "query": "我计划于2025年1月10日至12日从广州出发前往杭州，开启为期3天的旅行，总预算控制在14000元以内。旅行要求包括：全程入住五星级酒店，行程需覆盖钱塘江大桥、钱江龙、黄龙洞等经典景点，并安排一次黄焖鸡米饭与茶叶蛋的本地餐饮体验；10日早上出发，12日晚返回，控制预算同时兼顾景点品质、用餐体验与住宿舒适度，优先规划紧凑合理路线，在高性价比的同时尽量延长游玩时间、减少通勤和排队时间。",
