...
budget = 23000
start_date = "2025年02月11日"
end_date = "2025年02月15日"
travel_days = 5
peoples = 1
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    def obj_rule(model):
        ...
        transport_duration = sum(
            model.attr_hotel[d, a, h] * (
               ...
               for h in model.accommodations)
        return rating - transport_duration
    ...
    model.feature_rest1 = pyo.Constraint(
        rule=lambda m: sum(
            model.select_rest[d,r]
            for r in model.restaurants
            for d in days
            if '中式素菜馆' in model.rest_data[r]['type']
        ) >= 1
    )

    model.feature_duration = pyo.Constraint(
        rule=lambda m: sum(
            model.select_rest[d,r]
            for r in model.restaurants
            for d in days
...
def get_selected_poi(model, type, day, selected_poi, k=1):
    ...
    selected_poi = [
        poi_data[t]
        for t in poi_set
        if pyo.value(select_set[day, t]) > 0.9
    ]
    return selected_poi
...
def generate_poi(model, intra_city_trans):
    ...
    return {
        "query_id": "7",
        "query": "2025年02月11日至2025年02月15日，我计划从厦门市前往深圳市旅游，预算23000元，希望品尝当地的特色中式素菜馆，用餐时间与排队时间之和不能超过120分钟,每日行程安排需兼顾热门景点与合理动线,尽可能缩短市内通勤时间,整体旅行体验需体现高舒适性与服务品质。",
