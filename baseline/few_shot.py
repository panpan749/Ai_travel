...
def parse_parameters():
    ...
    parser.add_argument('--questoin_path', type=str, required=True, default='')
    parser.add_argument('--save_path', type=str, required=True, default='')
    parser.add_argument('--predict_path', type=str, required=True, default='')
    parser.add_argument('--model_name', type=str, required=True, default='')
    parser.add_argument('--api_key', type=str, required=True, default='')
    parser.add_argument('--base_url', type=str, required=True, default='')
...
5.代码必须以```code_begin开始,以```code_end结束
'''
...
def create_code_prompt(queston):
    prompt = f"#案例:\n{CODE_SAMPLES}\n```question_begin:\n{queston}\n```question_end\n```code_begin\n"
    return prompt
...
def main():
    ...
    code_dir = args.save_path
    predict_path = args.predict_path
    questoin_path = args.questoin_path
    save_data = []
    if not os.path.exists(code_dir):
        os.makedirs(code_dir)
    eval_set = load_json_file(questoin_path)
    try:
        nt = 0
        for sample in eval_set:
            ...
            print("正在输出第{}个问题的代码".format(nt))
            res = ask_llm(prompt, CODE_INSTRUCT, args.model_name)
            code = res.split('```code_begin')[1].split('```code_end')[0]
