...
budget = 12000
start_date = "2025年06月20日"
end_date = "2025年06月22日"
travel_days = 3
peoples = 4
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    def obj_rule(model):
        ...
        return -total_rating + 0.0001 * total_cost  # 主要优化评分，次要优化成本
...
def generate_daily_plan(model, intra_city_trans):
    ...
    for d in model.days:
        ...
        if d < travel_days:
            # 计算交通时间和费用
            taxi_duration = get_trans_params(intra_city_trans, selected_hotel['id'], selected_attr['id'], 'taxi_duration')
            bus_duration = get_trans_params(intra_city_trans, selected_hotel['id'], selected_attr['id'], 'bus_duration')
            taxi_cost = get_trans_params(intra_city_trans, selected_hotel['id'], selected_attr['id'], 'taxi_cost')
            bus_cost = get_trans_params(intra_city_trans, selected_hotel['id'], selected_attr['id'], 'bus_cost')
            
            transport_info = {
                'mode': transport_mode,
                'duration': taxi_duration * 2 if transport_mode == 'taxi' else bus_duration * 2,
                'cost': taxi_cost * 2 if transport_mode == 'taxi' else (peoples // 4) * bus_cost * 2
            }
        else:
            # 最后一天只有单程
            taxi_duration = get_trans_params(intra_city_trans, selected_hotel['id'], selected_attr['id'], 'taxi_duration')
            ...
            transport_info = {
                'mode': transport_mode,
                'duration': taxi_duration if transport_mode == 'taxi' else bus_duration,
                'cost': taxi_cost if transport_mode == 'taxi' else (peoples // 4) * bus_cost
            }
        
        # 计算每日总时间
        attr_time = selected_attr['duration']
        rest_time = sum(r['duration'] for r in selected_rests)
        transport_time = transport_info['duration']
        total_time = attr_time + rest_time + transport_time
...
def main():
    ...
    # 过滤出连锁快捷酒店
    chain_hotels = [h for h in poi_data['accommodations'] if '连锁' in h['type'] or '快捷' in h['type']]
    poi_data['accommodations'] = chain_hotels
