...
budget = 0  # 不限制预算
start_date = "2025年09月20日"
end_date = "2025年09月23日"
travel_days = 4
peoples = 3
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 过滤景点：只包含棠湖公园和锦里
    attractions = [a for a in poi_data['attractions'] if a['name'] in ['棠湖公园', '锦里']]
    ...
    # 过滤餐厅：火锅、串串等特色菜
    restaurants = [r for r in poi_data['restaurants'] if '火锅' in r['type'] or '串串' in r['type']]
    ...
    # 约束条件：每天必须选择1个景点
    def one_attraction_rule(model, d):
        return sum(model.select_attr[d, a] for a in model.attractions) == 1

    model.one_attraction = pyo.Constraint(model.days, rule=one_attraction_rule)
    ...
    # 约束条件：每天必须选择3个餐厅
    def three_restaurants_rule(model, d):
        return sum(model.select_rest[d, r] for r in model.restaurants) == 3

    model.three_restaurants = pyo.Constraint(model.days, rule=three_restaurants_rule)
    ...
    def obj_rule(model):
        ...
        return total_waiting + total_trans_time
