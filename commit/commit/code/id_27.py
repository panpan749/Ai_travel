...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 过滤景点：只包含八大关、栈桥、五四广场等地标景点
    target_attractions = ['八大关', '栈桥', '五四广场']
    attractions = [a for a in poi_data['attractions'] if a['name'] in target_attractions]
    
    # 过滤酒店：评分4.7以上、经济型连锁、价格低于1200元
    hotels = [h for h in poi_data['accommodations'] 
              if h['rating'] >= 4.7 and h['type'] == '经济型连锁' and h['cost'] < 1200]
    
    # 过滤餐厅：鲁菜和海鲜
    restaurants = [r for r in poi_data['restaurants'] 
                  if '鲁菜' in r['type'] or '海鲜' in r['type']]
    ...
    def time_constraint_rule(model, d):
        ...
        elif d == travel_days:  # 最后一天不考虑返程火车时间
            attr_time = sum(model.select_attr[d, a] * model.attr_data[a]['duration'] for a in model.attractions)
            rest_time = sum(model.select_rest[d, r] * (model.rest_data[r]['duration'] + model.rest_data[r]['queue_time']) for r in model.restaurants)
            transport_time = sum(
                model.attr_hotel[d, a, h] * (
    ...
    for d in model.days:
        ...
        if d == 1:
            day_plan['train'] = {
                ...
                'departure_station': model.train_departure_data[selected_train_departure]['origin_station'],
                'arrival_station': model.train_departure_data[selected_train_departure]['destination_station'],
                'duration': model.train_departure_data[selected_train_departure]['duration'],
                'cost': model.train_departure_data[selected_train_departure]['cost']
            }
        elif d == travel_days:
            day_plan['train'] = {
                'type': '返程',
                'train_number': model.train_back_data[selected_train_back]['train_number'],
                'departure_station': model.train_back_data[selected_train_back]['origin_station'],
                'arrival_station': model.train_back_data[selected_train_back]['destination_station'],
                'duration': model.train_back_data[selected_train_back]['duration'],
                'cost': model.train_back_data[selected_train_back]['cost']
            }
