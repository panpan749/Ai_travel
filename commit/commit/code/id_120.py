...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    def obj_rule(model):
        ...
        rating += sum(model.select_rest[d, r] * model.rest_data[r]['rating'] for d in model.days for r in model.restaurants)
        rating += sum(model.select_hotel[h] * model.hotel_data[h]['rating'] for h in model.accommodations)
        return -rating  # 最小化负评分即最大化评分
    ...
    for d in model.days:
        ...
        for a in model.attractions:
            if pyo.value(model.select_attr[d, a]) > 0.5:
                ...
                day_plan['total_time'] += model.attr_data[a]['duration']
                break
        ...
        for r in model.restaurants:
            if pyo.value(model.select_rest[d, r]) > 0.5:
                ...
                day_plan['total_time'] += model.rest_data[r]['duration']
        ...
        # 计算交通时间
        if d < travel_days:  # 最后一天不计算酒店到景点的交通
            for a in model.attractions:
                for h in model.accommodations:
                    if pyo.value(model.attr_hotel[d, a, h]) > 0.5:
                        if transport_mode == 'taxi':
                            time = (get_trans_params(intra_city_trans, h, a, 'taxi_duration') +
                                   get_trans_params(intra_city_trans, a, h, 'taxi_duration'))
                        else:
                            time = (get_trans_params(intra_city_trans, h, a, 'bus_duration') +
                                   get_trans_params(intra_city_trans, a, h, 'bus_duration'))
                        day_plan['transport_time'] = time
                        day_plan['total_time'] += time
                        break
