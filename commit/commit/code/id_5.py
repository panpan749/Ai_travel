...
def fetch_data():
    ...
    # 过滤酒店数据
    filtered_hotels = []
    for hotel in poi_data['accommodations']:
        if hotel['rating'] >= 4.5 and float(hotel['cost']) <= 900 and '市中心' in hotel['feature']:
            filtered_hotels.append(hotel)
    poi_data['accommodations'] = filtered_hotels

    # 过滤餐厅数据
    filtered_restaurants = []
    for restaurant in poi_data['restaurants']:
        if '小面' in restaurant['type'] or '火锅' in restaurant['type']:
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    model.trans_mode = pyo.Var(model.days, domain=pyo.Binary, initialize=1)  # 1表示公交，0表示打车
    ...
    # 约束条件：景点不重复
    def no_repeat_attraction_rule(model):
        return sum(model.select_attr[d1, a] * model.select_attr[d2, a] 
                  for a in model.attractions 
                  for d1 in model.days 
                  for d2 in model.days if d1 < d2) == 0

    model.no_repeat_attraction = pyo.Constraint(rule=no_repeat_attraction_rule)
    ...
    # 约束条件：餐厅不重复
    def no_repeat_restaurant_rule(model):
        return sum(model.select_rest[d1, r] * model.select_rest[d2, r] 
                  for r in model.restaurants 
                  for d1 in model.days 
                  for d2 in model.days if d1 < d2) <= 0

    model.no_repeat_restaurant = pyo.Constraint(rule=no_repeat_restaurant_rule)
    ...
    model.total_budget = pyo.Constraint(rule=total_budget_rule)
...
def generate_daily_plan(model, intra_city_trans):
    ...
    for d in model.days:
        ...
        # 计算交通时间
        if selected_hotel and day_plan['attraction']:
            hotel_id = None
            for h in model.accommodations:
                if pyo.value(model.select_hotel[h]) > 0.5:
                    hotel_id = h
            ...
            if hotel_id:
                ...
                if attr_id:
                    if transport_mode == '公交':
                        time = get_trans_params(intra_city_trans, hotel_id, attr_id, 'bus_duration') + \
                               get_trans_params(intra_city_trans, attr_id, hotel_id, 'bus_duration')
                    else:
                        time = get_trans_params(intra_city_trans, hotel_id, attr_id, 'taxi_duration') + \
                               get_trans_params(intra_city_trans, attr_id, hotel_id, 'taxi_duration')
