...
def fetch_data():
    ...
    filtered_accommodations = [h for h in all_accommodations if float(h['rating']) >= 4.8]
    
    # 过滤包含片儿川的餐厅
    all_restaurants = requests.get(url + f"/restaurants/{destination_city}").json()
    filtered_restaurants = [r for r in all_restaurants if "片儿川" in r.get('recommended_food', '')]
    
    poi_data = {
        'attractions': requests.get(url + f"/attractions/{destination_city}").json(),
        'accommodations': filtered_accommodations,
        'restaurants': filtered_restaurants if len(filtered_restaurants) >= 3*travel_days else all_restaurants
    }
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 餐厅不能在同一天重复
    def unique_rest_per_day_rule(model, d, r):
        return model.select_rest[d, r] <= 1

    model.unique_rest_per_day = pyo.Constraint(model.days, model.restaurants, rule=unique_rest_per_day_rule)
    ...
    def obj_rule(model):
        ...
        total_cost = model.budget_constraint.body - model.budget_constraint.upper
        return hotel_rating + attraction_rating + restaurant_rating + 0.01 * total_cost
