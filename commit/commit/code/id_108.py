    ...
    for d in model.days:
        ...
        for a in model.attractions:
            if pyo.value(model.select_attr[d, a]) > 0.5:
                selected_attraction = model.attr_data[a]
                break
        ...
        for r in model.restaurants:
            if pyo.value(model.select_rest[d, r]) > 0.5:
                selected_restaurants.append(model.rest_data[r])
        
        # 获取交通方式
        transport_mode = "taxi" if pyo.value(model.trans_mode[d]) < 0.5 else "bus"
        
        # 计算交通时间和费用
        taxi_duration = get_trans_params(intra_city_trans, selected_hotel['id'], selected_attraction['id'], 'taxi_duration')
        bus_duration = get_trans_params(intra_city_trans, selected_hotel['id'], selected_attraction['id'], 'bus_duration')
        taxi_cost = get_trans_params(intra_city_trans, selected_hotel['id'], selected_attraction['id'], 'taxi_cost')
        bus_cost = get_trans_params(intra_city_trans, selected_hotel['id'], selected_attraction['id'], 'bus_cost')
        
        day_plan['attraction'] = {
            'name': selected_attraction['name'],
            'cost': selected_attraction['cost'],
            'rating': selected_attraction['rating'],
            'duration': selected_attraction['duration']
        }
        ...
        day_plan['transport'] = {
            ...
            'cost': taxi_cost * 2 if transport_mode == 'taxi' else bus_cost * 2 * peoples
        }
        
        plan['days'][f'Day {d}'] = day_plan
