...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 约束：选择豪华套房
    def luxury_suite_rule(model):
        return sum(model.select_hotel[h] for h in model.accommodations if '豪华套房' in model.hotel_data[h]['type']) == 1

    model.luxury_suite = pyo.Constraint(rule=luxury_suite_rule)

    # 约束：选择海南鸡饭餐厅
    def hainan_chicken_rice_rule(model):
        return sum(model.select_rest[d, r] for d in model.days for r in model.restaurants 
                  if '海南鸡饭' in model.rest_data[r]['recommended_food']) >= 1

    model.hainan_chicken_rice = pyo.Constraint(rule=hainan_chicken_rice_rule)
...
def generate_daily_plan(model, intra_city_trans):
    ...
    for d in model.days:
        ...
        day_plan['transport_mode'] = '公交' if transport_mode > 0.5 else '出租车'
