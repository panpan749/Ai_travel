...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 过滤酒店：评分4.7以上
    hotels = [h for h in poi_data['accommodations'] if h['rating'] >= 4.7 and h['type'] == '民宿']
    # 过滤餐厅：推荐焗海鲜且人均消费180元内
    restaurants = [r for r in poi_data['restaurants'] if '焗海鲜' in r['recommended_food'] and r['cost'] <= 180]
...
def generate_daily_plan(model, intra_city_trans):
    ...
    # 火车票费用
    total_cost += peoples * (plan['departure']['cost'] + plan['return']['cost'])
    
    # 酒店费用
    total_cost += ((peoples + 1) // 2) * plan['accommodation']['cost'] * (travel_days - 1)
    
    # 景点费用
    attraction_cost = 0
    for day in daily_plans:
        attraction_cost += day['attraction']['cost']
        ...
        for a in model.attractions:
            if pyo.value(model.select_attr[d, a]) > 0.5:
                for h in model.accommodations:
                    if pyo.value(model.select_hotel[h]) > 0.5:
                        if mode == 'taxi':
                            taxi_cost = get_trans_params(intra_city_trans, h, a, 'taxi_cost') + \
                                       get_trans_params(intra_city_trans, a, h, 'taxi_cost')
                            transport_cost += taxi_cost
                        else:
                            bus_cost = peoples * (get_trans_params(intra_city_trans, h, a, 'bus_cost') + \
                                                 get_trans_params(intra_city_trans, a, h, 'bus_cost'))
                            transport_cost += bus_cost
