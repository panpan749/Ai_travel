...
budget = 30000
start_date = "2025年08月20日"
end_date = "2025年08月24日"
travel_days = 5
peoples = 3
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 筛选符合条件的POI
    attractions = [a for a in poi_data['attractions'] if a['id'] in ['大熊猫基地', '青羊宫']]
    hotels = [h for h in poi_data['accommodations'] if h['rating'] >= 4.7 and '早餐' in h['feature']]
    restaurants = [r for r in poi_data['restaurants'] if '串串香' in r['name'] and r['cost'] <= 200]
    ...
    def daily_time_rule(model, d):
        if d == travel_days:  # 最后一天不需要返回酒店
            return sum(model.select_attr[d, a] * model.attr_data[a]['duration'] for a in model.attractions) + \
                   ...
                   sum(model.attr_hotel[d-1, a, h] * (
                       (1 - model.trans_mode[d-1]) * get_trans_params(intra_city_trans, h, a, 'taxi_duration') + \
                       model.trans_mode[d-1] * get_trans_params(intra_city_trans, h, a, 'bus_duration')
    ...
    model.unique_attraction_constraint = pyo.Constraint(model.attractions, rule=unique_attraction_rule)
    ...
    model.unique_restaurant_constraint = pyo.Constraint(model.restaurants, rule=unique_restaurant_rule)
    ...
    for d in model.days:
        ...
        for a in model.attractions:
            if pyo.value(model.select_attr[d, a]) > 0.5:
                day_plan['attraction'] = a
                break
        
        # 获取餐厅
        for r in model.restaurants:
            if pyo.value(model.select_rest[d, r]) > 0.5:
                day_plan['restaurants'].append(r)
        
        # 获取交通方式
        day_plan['transport_mode'] = 'taxi' if pyo.value(model.trans_mode[d]) < 0.5 else 'bus'
        
        plan[f'Day {d}'] = day_plan
    
    # 添加火车信息
    plan['Departure Train'] = departure_train
    plan['Return Train'] = back_train
