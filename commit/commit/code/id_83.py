...
budget = 7800
start_date = "2025年09月18日"
end_date = "2025年09月21日"
travel_days = 4
peoples = 1
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 筛选符合条件的POI
    attractions = [a for a in poi_data['attractions'] if a['id'] in ['虎丘山', '留园']]
    hotels = [h for h in poi_data['accommodations'] if float(h['rating']) >= 4.6]
    restaurants = [r for r in poi_data['restaurants'] 
                  if float(r['cost']) <= 350 and '松鼠桂鱼' in r['recommended_food']]
...
def generate_daily_plan(model, intra_city_trans):
    ...
    for d in model.days:
        day_plan = {
            ...
            'restaurants': [],
            'transport_mode': None,
            'train': selected_train_departure if d == 1 else (selected_train_back if d == travel_days else None)
        }
