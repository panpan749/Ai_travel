...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    hotel_dict = {h['id']: h for h in poi_data['accommodations'] if float(h['cost']) <= 1300}  # 限制酒店价格
    ...
    def obj_rule(model):
        ...
        # 加权目标：主要考虑成本，次要考虑评分
        return total_cost - 0.1 * total_rating
...
def generate_daily_plan(model, intra_city_trans):
    ...
    for d in model.days:
        ...
        daily_plans[d]['restaurants'] = restaurants

        # 交通方式
        trans_mode = "taxi" if pyo.value(model.trans_mode[d]) < 0.5 else "bus"
        daily_plans[d]['transport_mode'] = trans_mode
