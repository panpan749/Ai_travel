...
def fetch_data():
    ...
    poi_data['restaurants'] = [r for r in poi_data['restaurants'] 
                              if '文昌鸡' in r['recommended_food'] and r['cost'] <= 300]
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 约束条件：每个景点只能被访问一次
    def attr_visit_once_rule(model, a):
        return sum(model.select_attr[d, a] for d in model.days) <= 1
    model.attr_visit_once = pyo.Constraint(model.attractions, rule=attr_visit_once_rule)
    ...
    # 约束条件：每个餐厅每天最多被选择一次
    def rest_visit_once_rule(model, r):
        return sum(model.select_rest[d, r] for d in model.days) <= 1
    model.rest_visit_once = pyo.Constraint(model.restaurants, rule=rest_visit_once_rule)
