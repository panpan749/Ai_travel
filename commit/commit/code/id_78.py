...
budget = 38000
start_date = "2025年08月15日"
end_date = "2025年08月19日"
travel_days = 5
peoples = 4
required_attractions = ["小湾河", "东山山体公园"]
required_restaurant_type = "酸汤鱼"
max_restaurant_cost = 150
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 过滤数据
    attractions = [a for a in poi_data['attractions'] if a['name'] in required_attractions or a['type'] == '自然']
    hotels = [h for h in poi_data['accommodations'] if h['rating'] >= 4.6]
    restaurants = [r for r in poi_data['restaurants'] if 
                  (required_restaurant_type in r['type'] or required_restaurant_type in r['recommended_food']) and 
                  r['cost'] <= max_restaurant_cost]
    ...
    # 目标函数：最大化游玩时间占比
    def obj_rule(model):
        total_attr_time = sum(model.select_attr[d, a] * model.attr_data[a]['duration'] 
                             for d in model.days for a in model.attractions)
        total_rest_time = sum(model.select_rest[d, r] * model.rest_data[r]['duration'] 
        ...
        return total_attr_time + total_rest_time - 0.1 * total_transport_time
    ...
    for d in model.days:
        ...
        if selected_attr and selected_hotel:
            if transport_mode == "taxi":
                ...
                trans_cost = peoples * (get_trans_params(intra_city_trans, selected_hotel['id'], selected_attr['id'], 'bus_cost') + \
                                       get_trans_params(intra_city_trans, selected_attr['id'], selected_hotel['id'], 'bus_cost'))
        ...
        daily_plan['attraction'] = selected_attr
        daily_plan['restaurants'] = selected_rests
        daily_plan['transport_mode'] = transport_mode
        daily_plan['transport_cost'] = trans_cost
        daily_plan['transport_time'] = trans_time
        
        daily_plans[f"Day {d}"] = daily_plan
