...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 过滤景点：必须包含栈桥和八大关
    attractions = [a for a in poi_data['attractions'] if a['name'] in ['栈桥', '八大关']]
    attraction_dict = {a['id']: a for a in attractions}
    
    # 过滤酒店：经济型连锁且评分4.5以上
    hotels = [h for h in poi_data['accommodations'] if h['type'] == '经济型连锁' and float(h['rating']) >= 4.5]
    hotel_dict = {h['id']: h for h in hotels}
    
    # 过滤餐厅：推荐锅贴且人均消费400元内
    restaurants = [r for r in poi_data['restaurants'] if '锅贴' in r.get('recommended_food', '') and float(r['cost']) <= 400]
    ...
    def obj_rule(model):
        ...
        total_cost = (hotel_cost + transport_cost + 
                      peoples * (attraction_cost + restaurant_cost + train_departure_cost + train_back_cost))
        
        return -total_rating / total_cost  # 最小化负的性价比
    ...
    def budget_rule(model):
        ...
        train_back_cost = sum(model.select_train_back[t] * model.train_back_data[t]['cost']
                              for t in model.train_back)
        return (hotel_cost + transport_cost + 
                peoples * (attraction_cost + restaurant_cost + train_departure_cost + train_back_cost)) <= budget
