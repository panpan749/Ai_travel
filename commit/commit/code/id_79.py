...
budget = 50000
start_date = "2025年06月18日"
end_date = "2025年06月20日"
travel_days = 3
peoples = 1
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 筛选符合条件的POI
    attractions = [a for a in poi_data['attractions'] if a['id'] in ['锦里', '大慈寺']]
    hotels = [h for h in poi_data['accommodations'] if float(h['rating']) >= 4.5]
    restaurants = [r for r in poi_data['restaurants'] 
                   if float(r['cost']) <= 200 and '麻辣火锅' in r['recommended_food']]
    ...
    model.trans_mode = pyo.Var(model.days, domain=pyo.Binary, bounds=(1,1))  # 强制使用公交
