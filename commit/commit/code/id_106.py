...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    hotel_dict = {h['id']: h for h in poi_data['accommodations'] if "海边" in h.get('feature', '')}
    restaurant_dict = {r['id']: r for r in poi_data['restaurants'] if "沙姜猪手" in r.get('recommended_food', '')}
    ...
    # 目标函数：最大化游玩时间，最小化排队和通勤时间
    def obj_rule(model):
        total_play_time = sum(model.select_attr[d, a] * model.attr_data[a]['duration'] for d in model.days for a in model.attractions)
        total_queue_time = sum(model.select_rest[d, r] * model.rest_data[r]['queue_time'] for d in model.days for r in model.restaurants)
        total_trans_time = sum(model.attr_hotel[d, a, h] * (
            (1 - model.trans_mode[d]) * (get_trans_params(intra_city_trans, h, a, 'taxi_duration') + 
                                         get_trans_params(intra_city_trans, a, h, 'taxi_duration')) +
            model.trans_mode[d] * (get_trans_params(intra_city_trans, h, a, 'bus_duration') + 
                                   get_trans_params(intra_city_trans, a, h, 'bus_duration'))
        ) for d in model.days for a in model.attractions for h in model.accommodations)
        return -total_play_time + total_queue_time + total_trans_time
