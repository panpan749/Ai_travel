...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 筛选四星级以上酒店
    accommodations = [h for h in poi_data['accommodations'] if float(h['rating']) >= 4]
    # 筛选本帮菜餐厅
    restaurants = [r for r in poi_data['restaurants'] if '本帮菜' in r['type']]
    if len(restaurants) < 3 * travel_days:  # 如果本帮菜不够，补充其他类型餐厅
        other_restaurants = [r for r in poi_data['restaurants'] if '本帮菜' not in r['type']]
        restaurants.extend(other_restaurants[:3*travel_days - len(restaurants)])
    ...
    def obj_rule(model):
        ...
        return rating_score + total_cost
...
def generate_daily_plan(model, intra_city_trans):
    ...
    for d in model.days:
        ...
        if selected_attr and selected_hotel:
            ...
            if key in intra_city_trans:
                trans_data = intra_city_trans[key]
                if transport_mode == "taxi":
                    trans_cost = float(trans_data['taxi_cost']) * 2  # 往返
                    trans_time = float(trans_data['taxi_duration']) * 2
                else:
                    trans_cost = float(trans_data['bus_cost']) * 2 * peoples
                    trans_time = float(trans_data['bus_duration']) * 2
            else:
                trans_cost = 0
                trans_time = 0
        else:
        ...
        # 计算当日总时间和费用
        attr_time = selected_attr['duration'] if selected_attr else 0
        rest_time = sum(r['duration'] for r in selected_rests) if selected_rests else 0
        total_time = attr_time + rest_time + trans_time
        
        attr_cost = selected_attr['cost'] if selected_attr else 0
        rest_cost = sum(r['cost'] for r in selected_rests) if selected_rests else 0
        total_cost = attr_cost + rest_cost + trans_cost
