...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    hotel_dict = {h['id']: h for h in poi_data['accommodations'] if '早餐' in h['feature']}  # 筛选提供早餐的酒店
    restaurant_dict = {r['id']: r for r in poi_data['restaurants'] if '东坡肉' in r['recommended_food']}  # 筛选有东坡肉的餐厅
    ...
    # 必须参观思澄堂
    sicheng_id = next(a['id'] for a in poi_data['attractions'] if a['name'] == '思澄堂')
    def sicheng_rule(model):
        return sum(model.select_attr[d, sicheng_id] for d in model.days) == 1
    model.sicheng_constraint = pyo.Constraint(rule=sicheng_rule)
    ...
    def obj_rule(model):
        ...
        return -total_rating + 0.001 * total_cost  # 加权平衡评分和成本
...
def generate_daily_plan(model, intra_city_trans):
    ...
    for d in days:
        ...
        daily_plan['transport_mode'].append(transport_mode)
    ...
    return {
        ...
        'hotel_cost_per_night': selected_hotel['cost'],
        'total_hotel_cost': selected_hotel['cost'] * (travel_days - 1) * ((peoples + 1) // 2),
        'daily_plans': plan,
