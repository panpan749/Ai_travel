...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    def daily_time_constraint_rule(model, d):
        total_time = 0
        # 景点时间
        total_time += sum(model.select_attr[d, a] * model.attr_data[a]['duration'] for a in model.attractions)
        # 餐厅时间
        total_time += sum(model.select_rest[d, r] * (model.rest_data[r]['duration'] + model.rest_data[r]['queue_time']) for r in model.restaurants)
        # 交通时间
        for a in model.attractions:
            for h in model.accommodations:
                taxi_time = get_trans_params(intra_city_trans, h, a, 'taxi_duration')
                bus_time = get_trans_params(intra_city_trans, h, a, 'bus_duration')
                total_time += model.attr_hotel[d, a, h] * (
                    (1 - model.trans_mode[d]) * (taxi_time * 2) + 
                    model.trans_mode[d] * (bus_time * 2)
                )
        return total_time <= 840
...
def generate_daily_plan(model, intra_city_trans):
    ...
    for d in model.days:
        day_plan = {"day": d, "attraction": None, "restaurants": [], "hotel": hotel_name if d != travel_days else None}
