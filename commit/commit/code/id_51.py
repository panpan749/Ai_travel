...
budget = 7000
start_date = "2025年06月08日"
end_date = "2025年06月11日"
travel_days = 4
peoples = 1
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 过滤餐厅
    restaurants = [r for r in poi_data['restaurants'] if '松鼠桂鱼' in r['recommended_food'] and r['cost'] <= 400]
    ...
    model.time_constraint = pyo.Constraint(model.days, rule=time_rule)
    ...
    def obj_rule(model):
        ...
        total_rating = hotel_rating + attr_rating + rest_rating
...
def generate_daily_plan(model, intra_city_trans):
    ...
    total_cost = 0
    # 火车费用
    total_cost += departure_train['cost'] + back_train['cost']
    # 酒店费用
    total_cost += selected_hotel['cost'] * (travel_days - 1)
    # 景点费用
    for day in plan:
        total_cost += day['attractions'][0]['cost']
        for rest in day['restaurants']:
            total_cost += rest['cost']
    
    # 交通费用
    for day in plan:
        attr_id = next(a['id'] for a in model.attr_data.values() if a['name'] == day['attractions'][0]['name'])
        hotel_id = next(h for h in model.accommodations if pyo.value(model.select_hotel[h]) > 0.5)
        
        if day['transport_mode'] == '公交':
            cost = get_trans_params(intra_city_trans, hotel_id, attr_id, 'bus_cost')
            total_cost += cost * 2  # 往返
        else:
