...
budget = 50000
start_date = "2025年07月28日"
end_date = "2025年08月01日"
travel_days = 5
peoples = 4
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 筛选符合条件的POI
    attraction_dict = {a['id']: a for a in poi_data['attractions'] if a['name'] in ['千古情景区', '红色娘子军']}
    hotel_dict = {h['id']: h for h in poi_data['accommodations'] if float(h['rating']) >= 4.7}
    restaurant_dict = {r['id']: r for r in poi_data['restaurants'] if '文昌鸡' in r['recommended_food'] and float(r['cost']) <= 200}
...
def generate_daily_plan(model, intra_city_trans):
    ...
    plan.append(f"出发交通: {train_departure['train_number']} 从 {train_departure['origin_station']} 到 {train_departure['destination_station']}, 耗时 {train_departure['duration']} 分钟")
    plan.append(f"住宿: {selected_hotel['name']} (评分: {selected_hotel['rating']})")
    ...
    for d in model.days:
        ...
        for a in model.attractions:
            if pyo.value(model.select_attr[d, a]) > 0.5:
                ...
                break
        if selected_attr:
            daily_plan.append(f"景点: {selected_attr['name']} (评分: {selected_attr['rating']}, 时长: {selected_attr['duration']} 分钟)")
        ...
        if selected_rests:
            rest_str = ", ".join([f"{r['name']} (推荐: {r['recommended_food']})" for r in selected_rests])
            daily_plan.append(f"餐厅: {rest_str}")
    ...
    plan.append(f"\n返程交通: {train_back['train_number']} 从 {train_back['origin_station']} 到 {train_back['destination_station']}, 耗时 {train_back['duration']} 分钟")
    return "\n".join(plan)
