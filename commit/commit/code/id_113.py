...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    hotel_dict = {h['id']: h for h in poi_data['accommodations'] if h['type'] == '连锁式酒店'}
    ...
    def daily_time_rule(model, d):
        ...
        trans_time = sum(model.attr_hotel[d, a, h] * (
            (1 - model.trans_mode[d]) * (get_trans_params(intra_city_trans, h, a, 'taxi_duration') + get_trans_params(intra_city_trans, a, h, 'taxi_duration')) +
            model.trans_mode[d] * (get_trans_params(intra_city_trans, h, a, 'bus_duration') + get_trans_params(intra_city_trans, a, h, 'bus_duration'))
    ...
    def obj_rule(model):
        ...
        return - (attr_score + rest_score + hotel_score)  # 最小化负分数等价于最大化分数
    ...
    def budget_rule(model):
        ...
        total_cost = (peoples + 1) // 2 * hotel_cost + transport_cost + peoples * (attraction_cost + restaurant_cost + train_departure_cost + train_back_cost)
