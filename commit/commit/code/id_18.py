...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    restaurant_dict = {r['id']: r for r in poi_data['restaurants'] if r['type'] in ['川菜馆', '快餐厅']}
    ...
    def attractions_unique_rule(model, a):
        return sum(model.select_attr[d, a] for d in model.days) <= 1

    model.attractions_unique = pyo.Constraint(model.attractions, rule=attractions_unique_rule)
    ...
    def restaurants_unique_rule(model, r):
        return sum(model.select_rest[d, r] for d in model.days) <= 1

    model.restaurants_unique = pyo.Constraint(model.restaurants, rule=restaurants_unique_rule)
    ...
    def obj_rule(model):
        ...
        # 权重设为0.01以平衡成本和评分
        return total_cost - 0.01 * (hotel_rating + attraction_rating + restaurant_rating)
...
def generate_daily_plan(model, intra_city_trans):
    plan = []
    days_order = ["第一天", "第二天", "第三天", "第四天", "第五天", "第六天"]
    ...
    day1_plan = {
        "日期": days_order[0],
        "出发交通": f"火车{selected_train_departure['train_number']}，从{selected_train_departure['origin_station']}到{selected_train_departure['destination_station']}，耗时{selected_train_departure['duration']}分钟",
        "景点": [],
        "餐饮": [],
        "住宿": selected_hotel['name'],
        "市内交通": []
    }
    ...
    for d in model.days:
        day_plan = {
            ...
            "景点": [],
            "餐饮": [],
            "住宿": selected_hotel['name'] if d < travel_days else "无",
            "市内交通": []
        }
        ...
        for a in model.attractions:
            if pyo.value(model.select_attr[d, a]) > 0.5:
                day_plan["景点"].append(model.attr_data[a]['name'])
                ...
                if transport_mode == "打车":
                    cost = get_trans_params(intra_city_trans, hotel_id, attr_id, 'taxi_cost')
                    duration = get_trans_params(intra_city_trans, hotel_id, attr_id, 'taxi_duration')
                else:
                    cost = get_trans_params(intra_city_trans, hotel_id, attr_id, 'bus_cost') * peoples
                    duration = get_trans_params(intra_city_trans, hotel_id, attr_id, 'bus_duration')
                
                day_plan["市内交通"].append(f"从酒店到{model.attr_data[a]['name']}: {transport_mode}, 花费{cost}元, 耗时{duration}分钟")
                
                # 回酒店（如果不是最后一天）
                if d < travel_days:
                    if transport_mode == "打车":
                        cost = get_trans_params(intra_city_trans, attr_id, hotel_id, 'taxi_cost')
                        duration = get_trans_params(intra_city_trans, attr_id, hotel_id, 'taxi_duration')
                    else:
                        cost = get_trans_params(intra_city_trans, attr_id, hotel_id, 'bus_cost') * peoples
                        duration = get_trans_params(intra_city_trans, attr_id, hotel_id, 'bus_duration')
                    
                    day_plan["市内交通"].append(f"从{model.attr_data[a]['name']}回酒店: {transport_mode}, 花费{cost}元, 耗时{duration}分钟")
                
        # 餐饮
        for r in model.restaurants:
            if pyo.value(model.select_rest[d, r]) > 0.5:
                day_plan["餐饮"].append(model.rest_data[r]['name'])
        
        if d == 1:
            day1_plan.update(day_plan)
            plan.append(day1_plan)
        else:
            plan.append(day_plan)
    
    # 最后一天：返程
    plan[-1]["返程交通"] = f"火车{selected_train_back['train_number']}，从{selected_train_back['origin_station']}到{selected_train_back['destination_station']}，耗时{selected_train_back['duration']}分钟"
