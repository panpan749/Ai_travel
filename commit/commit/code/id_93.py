...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    attraction_dict = {a['id']: a for a in poi_data['attractions'] if a['id'] == '世界之窗'}
    hotel_dict = {h['id']: h for h in poi_data['accommodations'] if float(h['rating']) >= 4.7}
    restaurant_dict = {r['id']: r for r in poi_data['restaurants'] if '粿条' in r.get('recommended_food', '')}
    ...
    def obj_rule(model):
        ...
        return total_cost - 0.01 * total_rating  # 最小化成本同时考虑评分
    ...
    # 餐厅人均消费限制
    def rest_cost_rule(model):
        return sum(
            model.select_rest[d, r] * model.rest_data[r]['cost']
            for d in model.days for r in model.restaurants
        ) <= 320 * 3 * peoples * travel_days
    model.rest_cost_constraint = pyo.Constraint(rule=rest_cost_rule)
...
def generate_daily_plan(model, intra_city_trans):
    ...
    for d in model.days:
        ...
        for a in model.attractions:
            if pyo.value(model.select_attr[d, a]) > 0.5:
                daily_plan['attraction'] = model.attr_data[a]
                break
        
        # 获取餐厅
        for r in model.restaurants:
            if pyo.value(model.select_rest[d, r]) > 0.5:
                daily_plan['restaurants'].append(model.rest_data[r])
    ...
    full_plan = {
        ...
        'peoples': peoples,
        'budget': budget,
        'departure_train': selected_train_departure,
        'return_train': selected_train_back,
        'daily_plans': plan
    }
    
    return json.dumps(full_plan, ensure_ascii=False, indent=2)
