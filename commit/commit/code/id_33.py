...
travel_days = 5
peoples = 2
hotel_rating_threshold = 4.8
hotel_price_threshold = 1000
restaurant_price_threshold = 320 * 3 * travel_days * peoples  # 人均320元 * 3餐 * 天数 * 人数
...
def fetch_data():
    ...
    poi_data['accommodations'] = [
        ...
        if float(h['rating']) >= hotel_rating_threshold and float(h['cost']) <= hotel_price_threshold
    ]
    
    # 过滤餐厅数据（粤菜）
    poi_data['restaurants'] = [
        r for r in poi_data['restaurants'] 
        if any(food in r['recommended_food'] for food in ['烧鹅', '手工包子', '陈村粉'])
    ]
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    def obj_rule(model):
        ...
        return - (0.4 * attraction_score + 0.2 * hotel_score + 0.4 * restaurant_score) + 0.1 * transport_time
...
def generate_daily_plan(model, intra_city_trans):
    ...
    plan['transport'] = {
        ...
        'return': {
            ...
            'departure_station': return_train['origin_station'],
            'arrival_station': return_train['destination_station'],
