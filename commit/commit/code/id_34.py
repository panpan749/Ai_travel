...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 约束条件：酒店评分高于4.5且价格不超过1500
    def hotel_quality_rule(model, h):
        return model.hotel_data[h]['rating'] >= 4.5 and model.hotel_data[h]['cost'] <= 1500

    model.hotel_quality = pyo.Constraint(model.accommodations, rule=hotel_quality_rule)
    ...
    def obj_rule(model):
        ...
        hotel_score = sum(model.select_hotel[h] * model.hotel_data[h]['rating'] 
                         for h in model.accommodations)
        return -(attraction_score + restaurant_score + hotel_score)
...
def generate_daily_plan(model, intra_city_trans):
    ...
    for d in model.days:
        ...
        # 获取交通方式
        if pyo.value(model.trans_mode[d]) > 0.5:
            day_plan["transport_mode"] = "bus"
        else:
            day_plan["transport_mode"] = "taxi"
