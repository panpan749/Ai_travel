...
def fetch_data():
    ...
    # 过滤餐馆只保留中式素菜馆
    filtered_restaurants = [r for r in poi_data['restaurants'] if r['type'] == '中式素菜馆']
    ...
    def obj_rule(model):
        ...
        transport_cost = sum(
            model.attr_hotel[d, a, h] * (
                    ...
                    model.trans_mode[d] * 4 * (  # 打车一次可载4人，按4人平摊
        ...
        # 最大化评分，最小化成本
        return -total_rating + 0.001 * total_cost
...
def generate_daily_plan(model, intra_city_trans):
    ...
    for d in model.days:
        day_plan = {
            'hotel': selected_hotel if d < travel_days else None,  # 最后一天不入住酒店
            'attraction': None,
            'restaurants': [],
            'transport_mode': None,
            'transport_time': 0
        ...
        # 计算交通时间
        if day_plan['attraction'] and selected_hotel:
            a_id = day_plan['attraction']['id']
            h_id = selected_hotel['id']
            if transport_mode == 'taxi':
                day_plan['transport_time'] = get_trans_params(intra_city_trans, h_id, a_id, 'taxi_duration') + \
                                           get_trans_params(intra_city_trans, a_id, h_id, 'taxi_duration')
            else:
                day_plan['transport_time'] = get_trans_params(intra_city_trans, h_id, a_id, 'bus_duration') + \
                                           get_trans_params(intra_city_trans, a_id, h_id, 'bus_duration')
        
        plan[f'Day {d}'] = day_plan
    
    # 添加火车信息
    plan['train_departure'] = train_departure
    plan['train_back'] = train_back
