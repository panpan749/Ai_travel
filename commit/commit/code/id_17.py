...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 过滤景点
    target_attractions = ["世界之窗", "欢乐港湾摩天轮", "深圳湾公园"]
    attractions = [a for a in poi_data['attractions'] if a['name'] in target_attractions]
    attraction_dict = {a['id']: a for a in attractions}
    
    # 过滤酒店
    hotels = [h for h in poi_data['accommodations'] if h['rating'] >= 4.5 and h['cost'] <= 800]
    hotel_dict = {h['id']: h for h in hotels}
    
    # 过滤餐厅
    target_cuisines = ["粤菜", "肠粉", "早茶"]
    restaurants = [r for r in poi_data['restaurants'] 
                  if any(cuisine in r['type'] for cuisine in target_cuisines) and r['cost'] <= 300]
    ...
    # 强制使用公共交通
    def prefer_public_transport_rule(model, d):
        return model.trans_mode[d] == 1

    model.prefer_public_transport = pyo.Constraint(model.days, rule=prefer_public_transport_rule)
