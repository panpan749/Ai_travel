...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 过滤景点和住宿
    filtered_attractions = [a for a in poi_data['attractions'] if a['name'] in ['故宫', '颐和园', '天坛']]
    attraction_dict = {a['id']: a for a in filtered_attractions}
    filtered_hotels = [h for h in poi_data['accommodations'] if h['rating'] >= 4.7 and h['cost'] <= 850]
    hotel_dict = {h['id']: h for h in filtered_hotels}
    filtered_restaurants = [r for r in poi_data['restaurants'] if r['type'] == '京菜' and r['cost'] <= 150]
    ...
    def obj_rule(model):
        ...
        return 0.7 * (1 - total_rating) + 0.3 * (total_cost / budget)
    ...
    def daily_time_rule(model, d):
        if d == travel_days:  # 最后一天
            return sum(model.select_attr[d, a] * model.attr_data[a]['duration'] 
                       for a in model.attractions) <= 840
        else:
            return sum(model.select_attr[d, a] * model.attr_data[a]['duration'] 
                       for a in model.attractions) + \
                   sum(model.select_rest[d, r] * model.rest_data[r]['duration'] 
    ...
    model.hotel_selection_constraint = pyo.Constraint(rule=hotel_selection_rule)
    ...
    model.transport_mode_constraint = pyo.Constraint(model.days, rule=transport_mode_rule)
...
def generate_daily_plan(model, intra_city_trans):
    ...
    for d in days:
        ...
        day_plan['restaurants'] = day_restaurants
