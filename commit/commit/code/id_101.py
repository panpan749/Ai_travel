...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 筛选符合要求的酒店（评分>=4.3，每晚<=300元）
    filtered_hotels = [h for h in poi_data['accommodations'] if float(h['rating']) >= 4.3 and float(h['cost']) <= 300]
    # 筛选包含翰林院遗址的景点
    attractions = [a for a in poi_data['attractions'] if '翰林院遗址' in a['name']] + [a for a in poi_data['attractions'] if '翰林院遗址' not in a['name']]
    # 筛选提供北京烤鸭的餐厅
    restaurants = [r for r in poi_data['restaurants'] if '北京烤鸭' in r['recommended_food']] + [r for r in poi_data['restaurants'] if '北京烤鸭' not in r['recommended_food']]
    ...
    # 必须选择翰林院遗址
    def must_visit_rule(model):
        return sum(model.select_attr[d, a] for d in model.days 
                  for a in model.attractions if '翰林院遗址' in model.attr_data[a]['name']) >= 1
    model.must_visit = pyo.Constraint(rule=must_visit_rule)

    # 必须至少吃一次北京烤鸭
    def must_eat_rule(model):
        return sum(model.select_rest[d, r] for d in model.days 
                  for r in model.restaurants if '北京烤鸭' in model.rest_data[r]['recommended_food']) >= 1
    model.must_eat = pyo.Constraint(rule=must_eat_rule)
    ...
    def daily_time_rule(model, d):
        ...
        elif d == travel_days:  # 最后一天不考虑返程交通时间
            attr_time = sum(model.select_attr[d, a] * model.attr_data[a]['duration'] for a in model.attractions)
            rest_time = sum(model.select_rest[d, r] * (model.rest_data[r]['duration'] + model.rest_data[r]['queue_time']) for r in model.restaurants)
            transport_time = sum(
                model.attr_hotel[d, a, h] * (
    ...
    model.hotel = pyo.Constraint(rule=hotel_rule)

    # 只能选择一个出发车次
    def train_departure_rule(model):
        return sum(model.select_train_departure[t] for t in model.train_departure) == 1
    model.train_departure_con = pyo.Constraint(rule=train_departure_rule)

    # 只能选择一个返程车次
    def train_back_rule(model):
        return sum(model.select_train_back[t] for t in model.train_back) == 1
    model.train_back_con = pyo.Constraint(rule=train_back_rule)
    ...
    def obj_rule(model):
        ...
        transport_cost = sum(
            model.attr_hotel[d, a, h] * (
                    ...
                    (peoples // 4 + 1) * model.trans_mode[d] * (
        ...
        total_cost = (peoples // 2) * hotel_cost + transport_cost + peoples * (
                      attraction_cost + restaurant_cost + train_departure_cost + train_back_cost)
        
        # 加权评分和成本（评分权重为1000，成本权重为-1）
        return 1000 * (hotel_rating + attr_rating + rest_rating) - total_cost
    ...
    def budget_rule(model):
        ...
        train_back_cost = sum(model.select_train_back[t] * model.train_back_data[t]['cost']
                              for t in model.train_back)
        return (peoples // 2) * hotel_cost + transport_cost + peoples * (
