...
budget = 13000
start_date = "2025年04月22日"
end_date = "2025年04月24日"
travel_days = 3
peoples = 2
...
def fetch_data():
    ...
    # 过滤景点只保留狮子林和网师园
    filtered_attractions = [a for a in poi_data['attractions'] if a['name'] in ['狮子林', '网师园']]
    poi_data['attractions'] = filtered_attractions

    # 过滤酒店评分4.6以上且是特色客栈
    filtered_hotels = [h for h in poi_data['accommodations'] if float(h['rating']) >= 4.6 and '特色客栈' in h['type']]
    poi_data['accommodations'] = filtered_hotels

    # 过滤餐厅推荐松鼠桂鱼且人均消费<=230
    filtered_restaurants = [r for r in poi_data['restaurants'] 
                           if '松鼠桂鱼' in r['recommended_food'] and float(r['cost']) <= 230]
...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    def obj_rule(model):
        ...
        return -0.5 * (attr_rating + rest_rating + hotel_rating) + 0.5 * (transport_time + train_departure_time + train_back_time)
...
def generate_daily_plan(model, intra_city_trans):
    ...
    for d in model.days:
        ...
        days_plan[d] = {
            'attraction': model.attr_data[selected_attr]['name'],
            'restaurants': [model.rest_data[r]['name'] for r in selected_rests],
            'hotel': model.hotel_data[selected_hotel]['name'] if d < travel_days else None,
            'transport': trans_mode
        }
