...
def build_model(cross_city_train_departure, cross_city_train_back, poi_data, intra_city_trans):
    ...
    # 筛选符合要求的POI
    attractions = [a for a in poi_data['attractions'] if a['name'] in ["东山公园", "贵州乡愁"]]
    hotels = [h for h in poi_data['accommodations'] if h['rating'] >= 4.5]
    restaurants = [r for r in poi_data['restaurants'] if r['cost'] <= 140 and "铜锅牛肉" in r['recommended_food']]
    ...
    # 目标函数：最大化游玩时间，最小化就餐时间
    def obj_rule(model):
        total_attr_time = sum(model.select_attr[d, a] * model.attr_data[a]['duration'] for d in model.days for a in model.attractions)
        total_rest_time = sum(model.select_rest[d, r] * model.rest_data[r]['duration'] for d in model.days for r in model.restaurants)
        return total_rest_time - total_attr_time  # 最小化就餐时间，最大化游玩时间
...
def generate_daily_plan(model, intra_city_trans):
    ...
    plan["transport"] = {
        ...
        "return": {
            "train_number": return_train['train_number'],
            "cost": return_train['cost'],
            "duration": return_train['duration']
        }
    }
